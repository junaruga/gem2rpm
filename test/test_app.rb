...
=end
  ...
  def get_options(options_string = '')
    ...
    argv = options_string.split
    config.send(:handle_options, argv)
    config.options
  end

  def start_app(options_string = '')
    options = get_options(options_string)
    ...
    out, err = capture_io do
      is_success = Gem2Rpm::App.start(options)
    end

    message = "Command: gem2rpm #{options_string}\nout: #{out}\nerr: #{err}"
    yield(is_success, message)
    [out, err]
  end

  def assert_gem2rpm_command(options_string = '')
    start_app(options_string) do |is_success, message|
